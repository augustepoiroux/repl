...
# Parse argument for number of parallel jobs
if [[ $# -ge 1 ]]; then
    if [[ $1 =~ ^[0-9]+$ && $1 -ge 1 ]]; then
        PARALLEL_JOBS=$1
    else
        echo "Usage: $0 [number_of_parallel_jobs]"
        echo "number_of_parallel_jobs must be a positive integer"
        exit 1
    fi
fi
...
export TMPDIR LOG_DIR GREEN RED YELLOW BLUE NC PATCH_ARRAY
